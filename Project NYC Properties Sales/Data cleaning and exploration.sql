SELECT TOP 100 *
FROM Portfolio..properties_NYC

SELECT TOP 100 *
FROM Portfolio..sales_NYC
...
SELECT *
FROM Portfolio..sales_NYC AS sales
INNER JOIN Portfolio..properties_NYC AS properties ON sales.F1 = properties.F1 AND sales.[ADDRESS] = properties.[ADDRESS] 
...
ALTER TABLE Portfolio..properties_NYC
ADD ApartmentNumber_new nvarchar(255);

UPDATE Portfolio..properties_NYC
...
ALTER TABLE Portfolio..sales_NYC
ADD SaleDate_converted date;

UPDATE Portfolio..sales_NYC
...
--ALTER TABLE Portfolio..NYC_sales
...
FROM (SELECT sales.F1, properties.BOROUGH, properties.NEIGHBORHOOD, properties.[BUILDING CLASS CATEGORY], properties.[BLOCK], properties.[LOT],
	...
	ROW_NUMBER () OVER (PARTITION BY properties.ApartmentNumber_new, sales.SaleDate_converted, properties.Address_correct,
								properties.[LOT]  ORDER BY properties.Address_correct, sales.[SALE PRICE] DESC) ROWS_DUP
	FROM Portfolio..sales_NYC AS sales
	INNER JOIN Portfolio..properties_NYC AS properties ON sales.F1 = properties.F1 AND sales.[ADDRESS] = properties.[ADDRESS] ) Sub
